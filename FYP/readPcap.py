...
from nexmonCSIFrame import NexmonCSIFrame
from utilities import byteops, stringops, csiextract
...
class NEXBeamformReader():
    ...
    HEADER_SLOTS = ["timestamp", "rssi", "frame_control", "source_mac", "sequence_no", "core", "spatial_stream",
                    "channel_spec",
                    "chip"]
    ...
    def read_file(self, path: str, scaled: bool = False) -> CSIData:
        ...
        if self.fill_skipped_frames:
            ...
            empty_frame = NexmonCSIFrame(self.EMPTY_HEADER, empty_csi)
    ...
    def read_bfee(self, pcap_frame: PcapFrame, bandwidth: int,
                  remove_unusuable_subcarriers: bool = True) -> NexmonCSIFrame:
        ...
        if len(data) % 2 != 0:
            return NexmonCSIFrame(pcap_frame.payloadHeader, np.zeros((self.BW_SUBS[bandwidth], 1)))

        csiData = data.reshape(-1, 2)
        csi = csiData.astype(np.float32).view(np.complex64)

        if self.scaled:
            csi = csiextract.scale_csi_frame(csi, pcap_frame.payloadHeader["rssi"])
        ...
        return NexmonCSIFrame(pcap_frame.payloadHeader, csi)

    def read_bfee_batch(self, pcap_frames: list, bandwidth: int, rx_num: int = 1, tx_num: int = 1) -> NexmonCSIFrame:

        total_csi = np.zeros((tx_num, rx_num, self.BW_SUBS[bandwidth]), dtype=complex)

        frame_header = pcap_frames[0].header
        payload_header = pcap_frames[0].payload_header
        ...
        for pcap_frame in pcap_frames:
            ...
            if self.scaled:
                csi = csiextract.scale_csi_frame(csi, pcap_frame.payload_header["rssi"])

            core = pcap_frame.payload_header["core"]
            spatial_stream = pcap_frame.payload_header["spatial_stream"]

            total_csi[core][spatial_stream] = csi.flatten()

        return NexmonCSIFrame(payload_header, np.transpose(total_csi))
    ...
    def read_frames(self, frames: list, scaled: bool, bandwidth: int) -> list:
        ...
        if frames[0].payload_header["sequence_no"] == frames[-1].payload_header["sequence_no"]:
        ...
        for frame in frames:
            if frame.payload_header["sequence_no"] != current_sequence_no:
                if len(current_sequence) > 0:
                    sequences.append(current_sequence)

                current_sequence = [frame]
                current_sequence_no = frame.payload_header["sequence_no"]
            else:
                if frame.payload_header["core"] > max_core:
                    max_core = frame.payload_header["core"]

                if frame.payload_header["spatial_stream"] > max_spatial_stream:
                    max_spatial_stream = frame.payload_header["spatial_stream"]
